...
export function showToast(message: string, type: ToastType = 'info') {
  ...
  // Theme-aware colors & styling
  let icon = ''
  let border = ''
  let bg = ''
  let textColor = isDark ? '#F1F5F9' : '#0F2A4A'
  let closeColor = isDark ? 'rgba(255,255,255,0.4)' : 'rgba(15,42,74,0.4)'
  let closeHoverColor = isDark ? '#FFFFFF' : '#0F2A4A'
  let boxShadow = isDark
    ? '0 10px 25px rgba(0,0,0,0.6), inset 0 1px 0 rgba(255,255,255,0.1)'
    : '0 12px 32px rgba(15,42,74,0.12), 0 2px 6px rgba(15,42,74,0.06)'
  ...
  if (isDark) {
    ...
    if (type === 'success') {
      ...
      boxShadow = '0 10px 25px rgba(0,0,0,0.6), 0 0 15px rgba(16,185,129,0.2), inset 0 1px 0 rgba(255,255,255,0.1)'
    } else if (type === 'error') {
      ...
      boxShadow = '0 10px 25px rgba(0,0,0,0.6), 0 0 15px rgba(239,68,68,0.2), inset 0 1px 0 rgba(255,255,255,0.1)'
      ...
      bg = 'rgba(120, 53, 4, 0.92)'
      boxShadow = '0 10px 25px rgba(0,0,0,0.6), 0 0 15px rgba(245,158,11,0.2), inset 0 1px 0 rgba(255,255,255,0.1)'
    } else {
      ...
      boxShadow = '0 10px 25px rgba(0,0,0,0.6), 0 0 15px rgba(59,130,246,0.2), inset 0 1px 0 rgba(255,255,255,0.1)'
    }
  } else {
    // Crisp Modern Light Mode Elevation Styling
    bg = '#FFFFFF'
    if (type === 'success') {
      icon = `<svg width="16" height="16" viewBox="0 0 15 15" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M11.4669 3.72684C11.7175 3.96614 11.7347 4.36106 11.5053 4.62089L6.50529 10.2876C6.38531 10.4237 6.21396 10.5 6.03332 10.5C5.85268 10.5 5.68132 10.4237 5.56134 10.2876L3.56134 8.02089C3.332 7.76106 3.34917 7.36614 3.59976 7.12684C3.85036 6.88754 4.25055 6.9045 4.47989 7.16433L6.03332 8.92361L10.5484 3.77443C10.7777 3.5146 11.1779 3.49754 11.4669 3.72684Z" fill="#059669"/></svg>`
      border = '1px solid #A7F3D0'
      toast.style.borderLeft = '4px solid #10B981'
      textColor = '#064E3B'
    } else if (type === 'error') {
      icon = `<svg width="16" height="16" viewBox="0 0 15 15" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M7.49997 1.5C4.18626 1.5 1.5 4.18626 1.5 7.49997C1.5 10.8137 4.18626 13.5 7.49997 13.5C10.8137 13.5 13.5 10.8137 13.5 7.49997C13.5 4.18626 10.8137 1.5 7.49997 1.5ZM8.24997 4.49997V8.24997H6.74997V4.49997H8.24997ZM8.24997 9.74997V11.25H6.74997V9.74997H8.24997Z" fill="#DC2626"/></svg>`
      border = '1px solid #FECDD3'
      toast.style.borderLeft = '4px solid #EF4444'
      textColor = '#881337'
    } else if (type === 'warning') {
      icon = `<svg width="16" height="16" viewBox="0 0 15 15" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M7.49997 0.75L0.75 12.75H14.25L7.49997 0.75ZM8.24997 5.25V9H6.74997V5.25H8.24997ZM8.24997 9.75V11.25H6.74997V9.75H8.24997Z" fill="#D97706"/></svg>`
  ...
  closeButton.onmouseenter = () => closeButton.style.color = closeHoverColor
  closeButton.onmouseleave = () => closeButton.style.color = closeColor
  closeButton.onclick = () => {
    toast.style.transform = 'translateX(50px) scale(0.95)'
    toast.style.opacity = '0'
